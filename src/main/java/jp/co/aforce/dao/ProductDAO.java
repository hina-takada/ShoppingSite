...
public class ProductDAO extends DAO {
	 ...
	 * @throws Exception
	 */
	public boolean update(int productId,String name,int categoryId,int price,int count,String description) throws Exception {
		
		Connection con = getConnection();
		PreparedStatement ps;
		
		ps = con.prepareStatement
			("UPDATE products SET name = ?,price = ?,count = ?,category_id = ?, description = ?  WHERE product_id = ?");
			ps.setString(1, name);
			ps.setInt(2, price);
			ps.setInt(3, count);
			ps.setInt(4, categoryId);
